export default class TwdCombat extends Combat {
  /**
  ...
  async rollInitiative(ids, { formula = null, updateTurn = true, messageOptions = {} } = {}) {
    ...
    for (let [i, id] of ids.entries()) {
      ...
      if (drawn < 10) {
        ...
        if (!game.settings.get('twd', 'twdHideInitChat')) {
          // Determine the roll mode
          let rollMode = messageOptions.rollMode || game.settings.get('core', 'rollMode');
          if ((combatant.token.hidden || combatant.hidden) && rollMode === 'roll') {
            rollMode = 'gmroll';
          }
          let cardPath = `<div style="text-align: center;"><img width="125" height="175" src="systems/twd/images/cards/card-${broll.total}.png"></div>`;
          try {
            if (game.settings.get('twd-corerules', 'imported')) {
              cardPath = `<div style="text-align: center;"><img width="125" height="175" src="modules/twd-corerules/images/cards/card-${broll.total}.png"></div>`;
            }
          } catch (error) {
            try {
              if (game.settings.get('twd-starterset', 'imported')) {
                cardPath = `<div style="text-align: center;"><img width="125" height="175" src="modules/twd-starterset/images/cards/card-${broll.total}.png"></div>`;
              }
            } catch (error) { }
          }
