...
export default async function migrateFolders() {
    ...
    while (await game.settings.get('twd', 'ARPGSemaphore') === 'busy') {
        await delay(10000);
    ...
    await game.settings.set('twd', 'ARPGSemaphore', 'busy');
    logger.warn('Status: ', await game.settings.get('twd', 'ARPGSemaphore'), 'Core System Starting Upgrade');
...
async function allDone(moduleTitle) {
    ...
    await game.settings.set('twd', 'ARPGSemaphore', '');
    await game.settings.set(moduleKey, 'migrationVersion', game.system.version);
    game.journal.getName('MU/TH/ER Instructions.').show();

    logger.warn('Status: ', await game.settings.get('twd', 'ARPGSemaphore'), 'Core System Upgrade Completed');
