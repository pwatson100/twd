...
export class COMMON {
  ...
  static applySettings(settingsData, moduleKey = COMMON.DATA.name) {
    Object.entries(settingsData).forEach(([key, data]) => {
      game.settings.register(moduleKey, key, {
        name: COMMON.localize(`TWD.${key}.name`),
        hint: COMMON.localize(`TWD.${key}.hint`),
        ...data,
      });
    });
  }
  ...
  static addAbstract(cls, key) {
    if (!globalThis.game.twd.abstract) globalThis.game.twd.abstract = {};
    globalThis.game.twd.abstract[key] = cls;
  }
}
