...
import { twdrTableGet } from './rollTableData.js';
...
export class twdActorSheet extends ActorSheet {
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      classes: ['twd', 'sheet', 'actor', 'actor-sheet'],
      // template: 'systems/twd/templates/actor/actor-sheet.html',
      width: 800,
  ...
  get template() {
    const path = 'systems/twd/templates/actor/';
  ...
  async getData(options) {
    ...
    switch (this.actor.type) {
      ...
      case 'creature':
        data.rTables = twdrTableGet.rTableget();
        data.cTables = twdrTableGet.cTableget();
  ...
  async _characterData(actor) {
    ...
    for (let [a, abl] of Object.entries(aData.attributes)) {
      ...
      abl.label = CONFIG.TWD.attributes[a];
    }
  ...
  async _prepareItems(data) {
    ...
    const inventory = {
      weapon: { section: 'Weapons', label: game.i18n.localize('TWD.InventoryWeaponsHeader'), items: [], dataset: { type: 'weapon' } },
      item: { section: 'Items', label: game.i18n.localize('TWD.InventoryItemsHeader'), items: [], dataset: { type: 'item' } },
      armor: { section: 'Armor', label: game.i18n.localize('TWD.InventoryArmorHeader'), items: [], dataset: { type: 'armor' } },
    };
  ...
  async _prepareCrew(sheetData) {
    ...
    sheetData.actor.system.crew.occupants.sort((o1, o2) => {
      const pos1 = TWD.vehicle.crewPositionFlags.indexOf(o1.position);
      const pos2 = TWD.vehicle.crewPositionFlags.indexOf(o2.position);
  ...
  async _computeEncumbrance(totalWeight, actorData) {
    ...
    let enc = {
      max: actorData.actor.system.attributes.str.value + 2,
  ...
  activateListeners(html) {
    ...
    const itemContextMenu = [
      {
        name: game.i18n.localize('TWD.addToFLocker'),
      ...
      {
        name: game.i18n.localize('TWD.moveFromFlocker'),
        icon: '<i class="fas fa-archive"></i>',
        callback: (element) => {
          let item = this.actor.items.get(element.data('item-id'));
      ...
      {
        name: game.i18n.localize('TWD.EditItemTitle'),
      ...
      {
        name: game.i18n.localize('TWD.DeleteItem'),
    ...
    if (game.settings.get('twd', 'switchMouseKeys')) {
  ...
  async _onDropItemCreate(itemData) {
    ...
    const alwaysAllowedItems = TWD.physicalItems;
    const allowedItems = {
    ...
    if (!allowed) {
      const msg = game.i18n.format('TWD.NotifWrongItemType', {
        type: type,
  ...
  _inlineedit(event) {
    ...
    // console.log('twdActorSheet -> _inlineedit -> dataset', dataset);
  ...
  _supplyRoll(event) {
    ...
    const lTemp = 'TWD.' + dataset.spbutt;
    // If this is a power roll get the exact id of the item to process
    const tItem = dataset.id || 0;
    const label = game.i18n.localize(lTemp) + ' ' + game.i18n.localize('TWD.Supply');
  ...
  _dropCrew(actorId) {
    ...
    if (actorData.system.crew.passengerQty >= actorData.system.attributes.passengers.value) {
      return ui.notifications.warn(game.i18n.localize('TWD.fullCrew'));
    }
