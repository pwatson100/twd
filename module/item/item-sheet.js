 ...
 * @extends {ItemSheet}
 */
export class twdItemSheet extends ItemSheet {
  /** @override */
  static get defaultOptions() {
    return mergeObject(super.defaultOptions, {
      classes: ['twd', 'sheet', 'item', 'item-sheet'],
      width: 675,
  ...
  /** @override */
  get template() {
    const path = 'systems/twd/templates/item';
  ...
  async _prepareSystemData(data) {
    this.item.update({ img: 'systems/twd/images/icons/solar-system.svg' });
    let enrichedFields = [
  ...
  async _prepareAgendaData(data) {
    ...
    if (this.item.img == 'icons/svg/item-bag.svg' && this.item.img != this.img) {
      this.item.update({ img: 'systems/twd/images/icons/personal-agenda.png' });
    }
  ...
  async _prepareSpecialtyData(data) {
    ...
    this.item.update({ img: 'systems/twd/images/icons/cover-notext.png' });

  }

  async _prepareTalentData(data) {
    if (data.system.general.career.value === '1' || data.system.general.career.value === '') {
      this.item.update({ img: 'systems/twd/images/icons/sprint.svg' });
    } else {
      this.item.update({ img: 'systems/twd/images/icons/fire-dash.svg' });
    }
