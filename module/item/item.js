...
export class twdItem extends Item {
  ...
  async roll(right) {
    ...
    let template = 'systems/twd/templates/dialog/roll-all-dialog.html';
    // let roll;
    let r2Data = 0;
    let reRoll = false;
    ...
    let label = `${item.name} (` + game.i18n.localize('TWD.Damage') + ` : ${itemData.attributes.damage.value})`;
    ...
    if (right) {
      ...
      if (this.actor.type === 'character' || actorData.header.synthstress) {
            ...
            close: (html) => {
              if (confirmed) {
                ...
                if (itemData.header.type.value === '1') {
                  ...
                  yze.yzeRoll(hostile, blind, reRoll, label, r1Data, game.i18n.localize('TWD.Black'), r2Data, game.i18n.localize('TWD.Yellow'), actorid, itemid);
                  ...
                  //   const ilabel = game.i18n.localize('TWD.Power') + ' ' + game.i18n.localize('TWD.Supply');
                  //   const consUme = 'power';
        ...
        template = 'systems/twd/templates/dialog/roll-base-dialog.html';
        renderTemplate(template).then((dlg) => {
          new Dialog({
            title: game.i18n.localize('TWD.DialTitle1') + ' ' + label + ' ' + game.i18n.localize('TWD.DialTitle2'),
            content: dlg,
        ...
        if (itemData.header.type.value === '1') {
          ...
          for (let [index] of actorData.crew.occupants.entries()) {
            if (actorData.crew.occupants[index].position != 'PASSENGER') {
              ...
              options = options.TWDconcat(`<option value="${index}">${firer.name}</option>`);
            }
          }
          if (fCrew.length === 0) {
            return ui.notifications.warn(game.i18n.localize('TWD.noCrewAssigned'));
          }
          let template = `
          <form>
              <div class="form-group">
              <label>${game.i18n.localize('TWD.SelectFirer')}</label>
              <select id="FirerSelect" name="FirerSelect">${options}</select>
              </div>
              <div class="form-group">
              <label>${game.i18n.localize('TWD.BaseMod')}</label>
              <input type="text" id="modifier" name="modifier" value="0" autofocus="autofocus" />
              </div>
              <div class="form-group">
              <label>${game.i18n.localize('TWD.StressMod')}</label>
          ...
          new Dialog({
            ...
            close: (html) => {
              if (confirmed) {
                ...
                reRoll = false;
                hostile = 'character';
                yze.yzeRoll(hostile, blind, reRoll, label, r1Data, game.i18n.localize('TWD.Black'), r2Data, game.i18n.localize('TWD.Yellow'), actorid, itemid, tactorid);
                game.twd.rollArr.sCount = game.twd.rollArr.r1Six + game.twd.rollArr.r2Six;
              }
            },
          }).render(true);
